...
def run_generic_adcs_app_test():
    ...
    for n in range(GENERIC_ADCS_TEST_LOOP_COUNT):
        ...
        cmd(f"GENERIC_ADCS GENERIC_ADCS_NOOP_CC with CCSDS_LENGTH {n+2}") # Note +2 due to CCSDS already being +1
        get_adcs_hk()
        # check(f"GENERIC_ADCS GENERIC_ADCS_HK_TLM CMD_COUNT == {cmd_cnt}")
        # check(f"GENERIC_ADCS GENERIC_ADCS_HK_TLM CMD_ERR_COUNT == {cmd_err_cnt+1}")
    ...
    for n in range(6, 6 + GENERIC_ADCS_TEST_LOOP_COUNT):
        ...
        cmd_cnt = tlm("GENERIC_ADCS GENERIC_ADCS_HK_TLM CMD_COUNT")
        cmd_err_cnt = tlm("GENERIC_ADCS GENERIC_ADCS_HK_TLM CMD_ERR_COUNT")
        cmd(f"GENERIC_ADCS GENERIC_ADCS_NOOP_CC with CCSDS_FC {n+1}")
        get_adcs_hk()
        # check(f"GENERIC_ADCS GENERIC_ADCS_HK_TLM CMD_COUNT == {cmd_cnt}")
        # check(f"GENERIC_ADCS GENERIC_ADCS_HK_TLM CMD_ERR_COUNT == {cmd_err_cnt+1}")
