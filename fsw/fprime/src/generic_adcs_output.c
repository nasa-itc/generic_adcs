/*******************************************************************************
** Purpose:
**   This file implements the functions to output messages to the actuator
*applications.
**
*******************************************************************************/
...
static void
send_mtb_commands(double Mcmd[3], Generic_ADCS_DO_Trq_TlmPayload_t *DO,
                  GENERIC_TORQUER_All_Percent_On_cmd_t *MtbPctOnCmd);
static void mcmd_to_percent_direction(double Mcmd, uint8 *percent,
                                      uint8 *direction);
static void send_rw_commands(double Tcmd[3],
                             Generic_ADCS_DO_Rw_TlmPayload_t *DO,
                             GENERIC_RW_Cmd_t *RwCmd);

void Generic_ADCS_output_init(FILE *in, Generic_ADCS_DO_Tlm_Payload_t *DO) {
  char junk[512], newline;
  fscanf(in, "%[^\n]%[\n]", junk, &newline);
  fscanf(in, "%lf %lf %lf %lf%[^\n]%[\n]", &DO->Trq.qba[0], &DO->Trq.qba[1],
         &DO->Trq.qba[2], &DO->Trq.qba[3], junk, &newline);
  fscanf(in, "%[^\n]%[\n]", junk, &newline);
  fscanf(in, "%lf %lf %lf%[^\n]%[\n]", &DO->Rw.axis[0][0], &DO->Rw.axis[0][1],
         &DO->Rw.axis[0][2], junk, &newline);
  fscanf(in, "%lf %lf %lf%[^\n]%[\n]", &DO->Rw.axis[1][0], &DO->Rw.axis[1][1],
         &DO->Rw.axis[1][2], junk, &newline);
  fscanf(in, "%lf %lf %lf%[^\n]%[\n]", &DO->Rw.axis[2][0], &DO->Rw.axis[2][1],
         &DO->Rw.axis[2][2], junk, &newline);
}

void Generic_ADCS_output_to_actuators(
    const Generic_ADCS_GNC_Tlm_Payload_t *GNC,
    Generic_ADCS_DO_Tlm_Payload_t *DO,
    GENERIC_TORQUER_All_Percent_On_cmd_t *MtbPctOnCmd,
    GENERIC_RW_Cmd_t *RwCmd) {
...
static struct {
  ...
  uint8 PercentOn;
    ...
    {1, 0},
    {1, 0},
    {1, 0}}; // Keep current state so commands are only sent on change
static void
send_mtb_commands(double Mcmd[3], Generic_ADCS_DO_Trq_TlmPayload_t *DO,
                  GENERIC_TORQUER_All_Percent_On_cmd_t *MtbPctOnCmd) {
  QTxV(DO->qba, Mcmd, DO->Mcmd);
  mcmd_to_percent_direction(DO->Mcmd[0], &MtbPctOnCmd->PercentOn_0,
                            &MtbPctOnCmd->Direction_0);
  mcmd_to_percent_direction(DO->Mcmd[1], &MtbPctOnCmd->PercentOn_1,
                            &MtbPctOnCmd->Direction_1);
  mcmd_to_percent_direction(DO->Mcmd[2], &MtbPctOnCmd->PercentOn_2,
                            &MtbPctOnCmd->Direction_2);
  if ((MtbPctOnCmd->Direction_0 != CurrentMtb[0].Direction) ||
      ...
      (MtbPctOnCmd->Direction_2 != CurrentMtb[2].Direction) ||
      (MtbPctOnCmd->PercentOn_2 != CurrentMtb[2].PercentOn)) {
...
static int16 CurrentRw[] = {
    0, 0, 0}; // Keep current state so commands are only sent on change
static void send_rw_commands(double Tcmd[3],
                             Generic_ADCS_DO_Rw_TlmPayload_t *DO,
                             GENERIC_RW_Cmd_t *RwCmd) {
  int16 torque;
