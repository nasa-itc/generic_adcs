...
static void AD_imu(const Generic_ADCS_DI_Imu_Tlm_Payload_t *DI_IMU,
                   Generic_ADCS_AD_Imu_Tlm_Payload_t *AD_IMU);
static void AD_mag(const Generic_ADCS_DI_Mag_Tlm_Payload_t *DI_Mag,
                   Generic_ADCS_AD_Mag_Tlm_Payload_t *AD_Mag);
static void AD_sol(const Generic_ADCS_DI_Fss_Tlm_Payload_t *DI_FSS,
                   const Generic_ADCS_DI_Css_Tlm_Payload_t *DI_CSS,
                   Generic_ADCS_AD_Sol_Tlm_Payload_t *AD_Sol);
static void AD_st(const Generic_ADCS_DI_St_Tlm_Payload_t *DI_ST,
                  Generic_ADCS_AD_ST_Tlm_Payload_t *AD_Mag);
static void AD_to_GNC(const Generic_ADCS_AD_Tlm_Payload_t *AD,
                      Generic_ADCS_GNC_Tlm_Payload_t *GNC);
static void AC_bdot(Generic_ADCS_GNC_Tlm_Payload_t *GNC,
                    Generic_ADCS_AC_Bdot_Tlm_t *AC_bdot);
static void AC_sunsafe(Generic_ADCS_GNC_Tlm_Payload_t *GNC,
                       Generic_ADCS_AC_Sunsafe_Tlm_t *ACS);
static void AC_inertial(Generic_ADCS_GNC_Tlm_Payload_t *GNC,
                        Generic_ADCS_AC_Inertial_Tlm_t *ACS);
static void AC_h_mgmt(Generic_ADCS_GNC_Tlm_Payload_t *GNC);

void Generic_ADCS_init_attitude_determination_and_attitude_control(
    FILE *in, Generic_ADCS_AD_Tlm_Payload_t *AD,
  ...
  // AC Bdot
  fscanf(in, "%[^\n]%[\n]", junk, &newline);
  fscanf(in, "%lf %lf%[^\n]%[\n]", &ACS->Bdot.b_range, &ACS->Bdot.Kb, junk,
         &newline);
  // AC Sunsafe
  fscanf(in, "%[^\n]%[\n]", junk, &newline);
  fscanf(in, "%lf %lf %lf %lf %lf %lf%[^\n]%[\n]", &ACS->Sunsafe.Kp[0],
         &ACS->Sunsafe.Kp[1], &ACS->Sunsafe.Kp[2], &ACS->Sunsafe.Kr[0],
         &ACS->Sunsafe.Kr[1], &ACS->Sunsafe.Kr[2], junk, &newline);
  fscanf(in, "%lf %lf %lf %lf %lf %lf %lf%[^\n]%[\n]", &ACS->Sunsafe.sside[0],
         &ACS->Sunsafe.sside[1], &ACS->Sunsafe.sside[2], &ACS->Sunsafe.vmax,
         &ACS->Sunsafe.cmd_wbn[0], &ACS->Sunsafe.cmd_wbn[1],
  ...
  // AC inertial
  fscanf(in, "%[^\n]%[\n]", junk, &newline);
  fscanf(in, "%lf %lf %lf %lf %lf%[^\n]%[\n]", &ACS->Inertial.qbn_cmd[0],
         &ACS->Inertial.qbn_cmd[1], &ACS->Inertial.qbn_cmd[2],
         &ACS->Inertial.qbn_cmd[3], &ACS->Inertial.phiErr_max, junk, &newline);
  fscanf(in, "%lf %lf %lf %lf %lf %lf%[^\n]%[\n]", &ACS->Inertial.Kp[0],
         &ACS->Inertial.Kp[1], &ACS->Inertial.Kp[2], &ACS->Inertial.Kr[0],
         &ACS->Inertial.Kr[1], &ACS->Inertial.Kr[2], junk, &newline);
  fscanf(in, "%lf %lf %lf%[^\n]%[\n]", &ACS->Inertial.Ki[0],
         &ACS->Inertial.Ki[1], &ACS->Inertial.Ki[2], junk, &newline);
  // AC Momentum management
  fscanf(in, "%[^\n]%[\n]", junk, &newline);
  fscanf(in, "%lf %lf %lf %lf%[^\n]%[\n]", &GNC->Hmgmt.Kb, &GNC->Hmgmt.b_range,
         &GNC->Hmgmt.loFrac, &GNC->Hmgmt.hiFrac, junk, &newline);
}

void Generic_ADCS_execute_attitude_determination_and_attitude_control(
    const Generic_ADCS_DI_Tlm_Payload_t *DI, Generic_ADCS_AD_Tlm_Payload_t *AD,
    Generic_ADCS_GNC_Tlm_Payload_t *GNC, Generic_ADCS_AC_Tlm_Payload_t *ACS) {
...
static void AD_imu(const Generic_ADCS_DI_Imu_Tlm_Payload_t *DI_IMU,
  ...
  if (DI_IMU->valid) {
    ...
    if (AD_IMU->init == 0) {
      ...
      for (int i = 0; i < 3; i++) {
        AD_IMU->wbn[i] = AD_IMU->alpha * AD_IMU->wbn_prev[i] +
                         (1 - AD_IMU->alpha) * DI_IMU->wbn[i];
      }
    }
    for (int i = 0; i < 3; i++) {
...
static void AD_mag(const Generic_ADCS_DI_Mag_Tlm_Payload_t *DI_Mag,
                   Generic_ADCS_AD_Mag_Tlm_Payload_t *AD_Mag) {
  /* AD very simple for magnetometer... there is only one mag and no fusion with
   * anything else */
  for (int i = 0; i < 3; i++) {
...
static void AC_bdot(Generic_ADCS_GNC_Tlm_Payload_t *GNC,
                    Generic_ADCS_AC_Bdot_Tlm_t *ACS) {
...
static void AC_sunsafe(Generic_ADCS_GNC_Tlm_Payload_t *GNC,
  ...
  double u1[3] = {0.0, 0.0, 0.0},
         err_b[3] = {0.0, 0.0, 0.0}; /* angle error calculation parameteres */
  ...
  if (GNC->SunValid) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      u1[i] =
          Limit(ACS->Kp[i] / ACS->Kr[i] * ACS->therr[i], -ACS->vmax, ACS->vmax);
...
static void AC_inertial(Generic_ADCS_GNC_Tlm_Payload_t *GNC,
                        Generic_ADCS_AC_Inertial_Tlm_t *ACS) {
  int i;
  double qErrLimited[4] = {
      0.0, 0.0, 0.0, 0.0}; /* Initialize Error quaterion for internal use */
  double e_axis[3] = {
      0.0, 0.0, 0.0};  /* Initialze Eigen axis of the Body to Body quaternion*/
  ...
  /*..Compute attittude/rate errors, Apply PD Control Law and compute minimum
   * Torque margin */
  for (i = 0; i < 3; i++) {
    ACS->therr[i] = 2.0 * qErrLimited[i];
    ACS->sumtherr[i] = ACS->sumtherr[i] + ACS->therr[i];
    ACS->werr[i] = -GNC->wbn[i];
    ACS->Tcmd[i] = ACS->Kp[i] * ACS->therr[i] + ACS->Kr[i] * ACS->werr[i] +
                   ACS->Ki[i] * ACS->sumtherr[i];
  }
