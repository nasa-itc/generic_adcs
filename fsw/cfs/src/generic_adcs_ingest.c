/*******************************************************************************
** Purpose:
**   This file implements the functions to ingest messages from the sensor
*applications.
**
*******************************************************************************/
...
void Generic_ADCS_ingest_init(FILE *in, Generic_ADCS_DI_Tlm_Payload_t *DI) {
  ...
  // Magnetometer
  fscanf(in, "%[^\n]%[\n]", junk, &newline);
  fscanf(in, "%lf %lf %lf %lf%[^\n]%[\n]", &DI->Mag.qbs[0], &DI->Mag.qbs[1],
         &DI->Mag.qbs[2], &DI->Mag.qbs[3], junk, &newline);
  // Fine Sun Sensor
  fscanf(in, "%[^\n]%[\n]", junk, &newline);
  fscanf(in, "%lf %lf %lf %lf%[^\n]%[\n]", &DI->Fss.qbs[0], &DI->Fss.qbs[1],
         &DI->Fss.qbs[2], &DI->Fss.qbs[3], junk, &newline);
  // Coarse Sun Sensors
  fscanf(in, "%[^\n]%[\n]", junk, &newline);
  fscanf(in, "%lf %lf %lf %lf%[^\n]%[\n]", &DI->Css.Sensor[0].axis[0],
         &DI->Css.Sensor[0].axis[1], &DI->Css.Sensor[0].axis[2],
         &DI->Css.Sensor[0].scale, junk, &newline);
  fscanf(in, "%lf %lf %lf %lf%[^\n]%[\n]", &DI->Css.Sensor[1].axis[0],
         &DI->Css.Sensor[1].axis[1], &DI->Css.Sensor[1].axis[2],
         &DI->Css.Sensor[1].scale, junk, &newline);
  fscanf(in, "%lf %lf %lf %lf%[^\n]%[\n]", &DI->Css.Sensor[2].axis[0],
         &DI->Css.Sensor[2].axis[1], &DI->Css.Sensor[2].axis[2],
         &DI->Css.Sensor[2].scale, junk, &newline);
  fscanf(in, "%lf %lf %lf %lf%[^\n]%[\n]", &DI->Css.Sensor[3].axis[0],
         &DI->Css.Sensor[3].axis[1], &DI->Css.Sensor[3].axis[2],
         &DI->Css.Sensor[3].scale, junk, &newline);
  fscanf(in, "%lf %lf %lf %lf%[^\n]%[\n]", &DI->Css.Sensor[4].axis[0],
         &DI->Css.Sensor[4].axis[1], &DI->Css.Sensor[4].axis[2],
         &DI->Css.Sensor[4].scale, junk, &newline);
  fscanf(in, "%lf %lf %lf %lf%[^\n]%[\n]", &DI->Css.Sensor[5].axis[0],
         &DI->Css.Sensor[5].axis[1], &DI->Css.Sensor[5].axis[2],
         &DI->Css.Sensor[5].scale, junk, &newline);
  // Inertial Measurement Unit
  fscanf(in, "%[^\n]%[\n]", junk, &newline);
  fscanf(in, "%lf %lf %lf %lf%[^\n]%[\n]", &DI->Imu.qbs[0], &DI->Imu.qbs[1],
         &DI->Imu.qbs[2], &DI->Imu.qbs[3], junk, &newline);
  fscanf(in, "%lf %lf %lf%[^\n]%[\n]", &DI->Imu.pos[0], &DI->Imu.pos[1],
         &DI->Imu.pos[2], junk, &newline);
  // Reaction Wheels
  fscanf(in, "%[^\n]%[\n]", junk, &newline);
  double h_max[3] = {0.0, 0.0, 0.0};
  fscanf(in, "%lf %lf %lf %lf%[^\n]%[\n]", &DI->Rw.whl_axis[0][0],
         &DI->Rw.whl_axis[0][1], &DI->Rw.whl_axis[0][2], &h_max[0], junk,
         &newline);
  fscanf(in, "%lf %lf %lf %lf%[^\n]%[\n]", &DI->Rw.whl_axis[1][0],
         &DI->Rw.whl_axis[1][1], &DI->Rw.whl_axis[1][2], &h_max[1], junk,
         &newline);
  fscanf(in, "%lf %lf %lf %lf%[^\n]%[\n]", &DI->Rw.whl_axis[2][0],
         &DI->Rw.whl_axis[2][1], &DI->Rw.whl_axis[2][2], &h_max[2], junk,
         &newline);
  ...
  // Star Tracker
  fscanf(in, "%[^\n]%[\n]", junk, &newline);
  fscanf(in, "%lf %lf %lf %lf%[^\n]%[\n]", &DI->St.qbs[0], &DI->St.qbs[1],
         &DI->St.qbs[2], &DI->St.qbs[3], junk, &newline);
}

void Generic_ADCS_ingest_generic_mag(CFE_MSG_Message_t *Msg,
                                     Generic_ADCS_DI_Mag_Tlm_Payload_t *Mag) {
  GENERIC_MAG_Device_tlm_t *mag = (GENERIC_MAG_Device_tlm_t *)Msg;
  double msg_bvs[3] = {mag->Generic_mag.MagneticIntensityX,
                       mag->Generic_mag.MagneticIntensityY,
...
void Generic_ADCS_ingest_generic_fss(CFE_MSG_Message_t *Msg,
...
void Generic_ADCS_ingest_generic_css(CFE_MSG_Message_t *Msg,
...
void Generic_ADCS_ingest_generic_imu(CFE_MSG_Message_t *Msg,
                                     Generic_ADCS_DI_Imu_Tlm_Payload_t *Imu) {
  GENERIC_IMU_Device_tlm_t *imu = (GENERIC_IMU_Device_tlm_t *)Msg;
  double wsn[3] = {imu->Generic_imu.X_Data.AngularAcc,
                   imu->Generic_imu.Y_Data.AngularAcc,
                   imu->Generic_imu.Z_Data.AngularAcc};
  QxV(Imu->qbs, wsn, Imu->wbn);
  double acc[3] = {imu->Generic_imu.X_Data.LinearAcc,
                   imu->Generic_imu.Y_Data.LinearAcc,
                   imu->Generic_imu.Z_Data.LinearAcc};
  QxV(Imu->qbs, acc, Imu->acc);
  Imu->valid = 1;
}

void Generic_ADCS_ingest_generic_rw(CFE_MSG_Message_t *Msg,
...
void Generic_ADCS_ingest_generic_st(CFE_MSG_Message_t *Msg,
                                    Generic_ADCS_DI_St_Tlm_Payload_t *St) {
  GENERIC_STAR_TRACKER_Device_tlm_t *st =
      (GENERIC_STAR_TRACKER_Device_tlm_t *)Msg;

  St->valid = st->Generic_star_tracker.IsValid;
  double q[4] = {st->Generic_star_tracker.Q0, st->Generic_star_tracker.Q1,
                 st->Generic_star_tracker.Q2, st->Generic_star_tracker.Q3};
  QxQ(q, St->qbs, St->q);
}
